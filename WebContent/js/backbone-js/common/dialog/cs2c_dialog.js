(function() {
	window.Cs2c_Tab = Backbone.View.extend({
		...
		showTabIndex : function(index) {
			$(this.el).find('#' + this.options.tab_id).children().eq(index)
					.show().siblings().hide();
			$(this.el).find('.cs2c_tab_title span').eq(index).addClass(
					'cs2c_tabs_selected').siblings().removeClass(
					'cs2c_tabs_selected');
		}
	});
}());
	...
	window.Cs2c_Wizard = Backbone.View
			.extend({
				...
				render : function() {
					...
					// 只显示第一屏的内容
					$(this.el).find('.cs2c_wizard_dialog_ctx').find(
							'.dialog_wizard_1').show().siblings().hide();
				...
				createWizardCtx : function() {
					$(this.el).append(
							'<div class="cs2c_wizard_dialog_ctx"></div>');
					$(this.el).find('.cs2c_wizard_dialog_ctx').append(
							$('#' + this.options.wizard_id));
					$(this.el).find('.cs2c_wizard_dialog_ctx').height(
							this.options.height);
				},
				...
				createWizardButton : function() {
					...
					dialog_btn
							.prepend('<a class="l-btn cancel" href="#"><span class="dialog-btn-left">取消</span></a>');
					dialog_btn
							.prepend('<a class="l-btn ok" href="#"><span class="dialog-btn-left">完成</span></a>');
					...
					dialog_btn.find('.up').hide();
					// 判断是否显示【完成】按钮
					this.options.achievable ? dialog_btn.find('.ok').show()
							: dialog_btn.find('.ok').hide();
					// 判断是否显示【取消】按钮
					this.options.cancelable ? dialog_btn.find('.cancel').show()
							: dialog_btn.find('.cancel').hide();
				},
				...
				hideWizardStep : function(stepIndex) {
					...
					// 2、控制界面内容切换的显示
					$('#' + this.options.wizard_id).find(
							'.dialog_wizard_' + (stepIndex + 1)).hide();
				},
				componentDisplayController : function(pageNum) {
					...
					var banners = $(this.el).find('.cs2c_wizard_dialog_banner')
							.children();
					banners.eq(pageNum - 1).addClass('redText').siblings()
							.removeClass('redText');

					// 2、控制界面内容切换的显示
					$('#' + this.options.wizard_id).find(
							'.dialog_wizard_' + pageNum).show().siblings()
							.hide();

					// 3.控制按钮的显示：如果是最后一页，显示完成和取消按钮
					var wizardNum = $(this.el).find(".cs2c_wizard_dialog_ctx")
							.children().children().length;

					// 判断每一步的按钮显示状态
					if (pageNum === wizardNum) {
						// 最终的完成界面是否显示【上一步】按钮
						this.options.okHasUp ? dialog_btn.find('.up').show()
								: dialog_btn.find('.up').hide();
						dialog_btn.find('.down').hide();
						dialog_btn.find('.ok').show();
					} else if (pageNum === 1) {
...
(function() {
	...
	/**
	 * cs2c组建库的信息提示 by qianqian.yang 2013-4-1
	 */
	window.cs2c_Message = function(msg, posEl) {
		...
		pageWidth = (typeof posEl != "undefined") ? posEl[0].scrollWidth
				: pageWidth;
		var message_body = document.createElement("div");
		message_body.className = "cs2c_message";
		message_body.style.left = ((typeof posEl != "undefined" ? posEl
				.offset().left : 0) + (pageWidth - 300) / 2)
				+ "px";
	...
	/**
	 * cs2c组建库的确认提示 by qianqian.yang 2013-4-18
	 */
