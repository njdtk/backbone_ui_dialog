...
var PagerView = Backbone.View
		.extend({
			...
			reRender : function() {
				...
				$(this.model.get("parentEl")).next().find("input.pager_num").val(
						pageNo);
				$(this.model.get("parentEl")).next().find("span#pageCount").html(
						pageCount);
				$(this.model.get("parentEl")).next().find("span#startNo")
						.html(startNo);
			 ...
			 */
			doPageCss : function() {
				// $(".pagination-page-list option[value='"+
				// this.model.get("pageSize") + "']").attr("selected", true);
				$(this.model.get("parentEl")).next().find("a")
						.removeClass("clickabled").addClass("disabled");
				$(this.model.get("parentEl")).next().find("#fresh").removeClass(
						"disabled").addClass("clickabled");
				var pageCount = this.model.get("pageCount");
				var pageNo = this.model.get("pageNo");
				if (pageCount > 1) {
					if (pageNo < pageCount && pageNo > 1) {
						$(this.model.get("parentEl")).next().find("a")
								.removeClass("disabled").addClass("clickabled");
					}
					if (pageNo == 1) {
						$(this.model.get("parentEl")).next().find("#last")
								.removeClass("disabled").addClass("clickabled");
						$(this.model.get("parentEl")).next().find("#next")
								.removeClass("disabled").addClass("clickabled");
					}
					if (pageNo == pageCount) {
						$(this.model.get("parentEl")).next().find("#prev")
								.removeClass("disabled").addClass("clickabled");
						$(this.model.get("parentEl")).next().find("#fir").removeClass(
								"disabled").addClass("clickabled");
					}
				}
			},
			...
			doPage : function(e) {
				...
				if (elem === "next") {
					this.model.set({
						"pageNo" : pageNo < pageCount ? (pageNo + 1)
								: pageCount
					});
				}
				if (elem === "fresh") {
					$(this.model.get("parentEl")).next().find(".pager_load").addClass(
							"pager_loading");
					var pageNo_val = $(this.model.get("parentEl")).next().find(
							".pager_num").val();
					var patternNum = /^[0-9][0-9]*/;
					if (patternNum.test(pageNo_val)) {
						pageNo_val = Number(pageNo_val) > pageCount ? pageCount
								: (Number(pageNo_val) < 1 ? 1
										: Number(pageNo_val));
					} else {
					...
					this.model.set({
						"pageSize" : Number($(this.model.get("parentEl")).next().find(
								".pager_list option:selected").val()),
						"pageNo" : 0
					});
					this.model.set({
						"pageSize" : Number($(this.model.get("parentEl")).next().find(
			...
			autoRefresh : function() {
				...
				this.model.set({
					"pageSize" : Number($(this.model.get("parentEl")).next()
							.find(".pager_list option:selected").val()),
					"pageNo" : 1
				});
			},
