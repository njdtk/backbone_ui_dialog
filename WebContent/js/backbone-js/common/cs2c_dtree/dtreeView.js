...
var DtreeView = Backbone.View
		.extend({
			...
			initialize : function() {
				...
				/*
				 * //有线条 this.icon = { root : 'img/base.gif',
				 * 
				...
				// 无线条
				this.icon = {
					root : 'js/common/cs2c_dtree/img/base.gif',

					folder : 'js/common/cs2c_dtree/img/folder.gif',

					folderOpen : 'js/common/cs2c_dtree/img/folderopen.gif',

					page : 'js/common/cs2c_dtree/img/page.gif',

					empty : 'js/common/cs2c_dtree/img/empty.gif',

					line : 'js/common/cs2c_dtree/img/empty.gif',

					join : 'js/common/cs2c_dtree/img/empty.gif',

					joinBottom : 'js/common/cs2c_dtree/img/empty.gif',

					plus : 'js/common/cs2c_dtree/img/tree_arrows_close.gif',

					plusBottom : 'js/common/cs2c_dtree/img/tree_arrows_close.gif',

					minus : 'js/common/cs2c_dtree/img/tree_arrows_open.gif',

					minusBottom : 'js/common/cs2c_dtree/img/tree_arrows_open.gif',

					nlPlus : 'js/common/cs2c_dtree/img/nolines_plus.gif',

					nlMinus : 'js/common/cs2c_dtree/img/nolines_minus.gif'
				};
				this.render();
			...
			bindEvents : function() {
				...
				$(this.el).find(".dtree_node span").bind("mouseover",
						_(function(e) {
							this.addStyle(e);
						}).bind(this));
				...
				$(this.el).find(".dtree_node span").bind("mouseout",
						_(function(e) {
							this.removeStyle(e);
						}).bind(this));
				...
				$(this.el).find(".dtree_node span").bind("click",
						_(function(e) {
//							console.log("click0000");
							this.onItemClick(e);
						}).bind(this));
				...
				$(this.el).find("input[type='checkbox']").bind("click",
						_(function(e) {
			...
			render : function() {
				...
				// 页面渲染
				$(this.el).append(
						this.eletmpl(this.rootNodeIndex, this
								.getCurImgs(this.rootNodeIndex)));
				this.showChildren(this.rootNodeIndex);
				// this.renderRightMenu();
			},

			createDiv : function() {
				$(this.options.parentEl).append(
						"<div id='" + this.objName + "'></div>");
			},
			...
			toggleSelectStyle : function(e) {
				...
				if (eleType == "SPAN") {
					...
					$(this.el).find("div[name='div_node_" + curIndex + "']")
							.addClass("selected");
					...
					if (this.allNodes[curIndex].checked) {
						$(this.el)
								.find("div[name='div_node_" + curIndex + "']")
								.addClass("selected");
					} else {
						$(this.el)
								.find("div[name='div_node_" + curIndex + "']")
			...
			handleChildren : function(e) {
				...
				} else if ($(curImg).attr("src") === this.icon.minus
			...
			renderNodes : function(index) {
				 ...
				 * this.eletmpl(index,this.getCurImgs(index)); }
				 */
			...
			removeNodes : function(index) {
				...
				/*
				 * for(var i=0; i<this.allNodes[curIndex].childNum; i++){ var
				 * childInx=this.allNodes[curIndex].childList[i];
			...
			eletmpl : function(index, imgs) {
				...
				returnVal = "<div class='dtree_node' style='height:19px;' name='div_node_"
						+ index
						+ "' >"
						+ imgEles
						+ checkBoxEle
						...
						+ this.allNodes[index].content + '</span>' + '</div>';
				return returnVal;
			},
			...
			itemRender : function(index) {
				$(this.el).find("span#node_" + index).html(
						this.allNodes[index].content);
				this.shine(index);
			...
			getAllChildren : function(model) {
				...
				for ( var i = 0; i < childIndexList.length; i++) {
					...
					if (curNode.childNum > 0) {
						returnVal = returnVal.concat(this
								.getAllChildren(curNode));
					}
				}
			...
			addNode : function(model) {
				...
				while ($(this.el)
						.find("div[name='div_node_" + showIndex + "']").length == 0) {
			...
			shine : function(index) {
				...
				this.timerId1 = window.setInterval(_(this.shining).bind(this),
						1000, index);
				this.timerId2 = window.setInterval(_(this.closeShining).bind(
						this), 700, index);
			},
			...
			shining : function(index) {
				//console.log("shining")
				$(this.el).find("div[name='div_node_" + index + "']").addClass(
						"selected");
			...
			closeShining : function(index) {
				...
				$(this.el).find("div[name='div_node_" + index + "']")
						.removeClass("selected");
				this.timerCount++;
				if (this.timerCount == 2) {
